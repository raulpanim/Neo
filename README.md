...
`bot.py` is a Telegram front-end for the infrastructure-graph feature: send
it a domain and it replies with the same graph as a PNG, generated by
calling `build_infra_graph()` directly (no separate Flask server needed).
...
The token is read from the environment only — never hardcode it or commit
it to the repo.
